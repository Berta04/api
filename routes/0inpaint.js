...
import checkImageTelegramFetch from "../middlewares/checkImageTelegramFetch.js";
...
export default {
    type: "post",
    middlewares: [checkImageTelegramFetch],
    start: async (req, res) => {
...
/*try {
            let options = await fetch("http://0.0.0.0:7860/sdapi/v1/options", {
                method: "post",
                headers: {
                    'Content-Type': 'application/json; charset=utf-8',
                    Accept: 'application/json'
                ...
                body: JSON.stringify({
                    sd_model_checkpoint: "version3.0-inpainting.safetensors [4d69ae3b6e]"
                })
            })
            let imageResponse = await fetch("http://0.0.0.0:7860/sdapi/v1/img2img", {
                method: "post",
                headers: {
                    'Content-Type': 'application/json; charset=utf-8',
                    Accept: 'application/json'
                },
                body: JSON.stringify(inpaintSettings)
            })

            let imageJson = await imageResponse.json();

            const outputImageBuffer1 = Buffer.from(imageJson.images[0], "base64");
            //const outputImageBuffer2 = Buffer.from(imageJson.images[1], "base64");
            //const outputImageBuffer3 = Buffer.from(imageJson.images[1], "base64");
            await fs.writeFile(`./imageFolder/${key}/image1.jpg`, outputImageBuffer1);
            //await fs.writeFile(`./imageFolder/${key}/image2.jpg`, outputImageBuffer2);
            //await fs.writeFile(`./imageFolder/${key}/image3.jpg`, outputImageBuffer3);
        }
        catch(err) {
            console.error(err);
        }*/